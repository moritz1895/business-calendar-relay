 ...
 * otherwise register for classes whose constructors need per-calendar configuration
 * values ({@code String}/{@code URI}) rather than Spring-resolvable dependencies.
 *
 ...
 * {@link CalDavCalendarSourceAdapter} were never meant to be resolved as Spring beans by
 * type -- {@link RelayWiringConfiguration} constructs one instance of each per
 * configured calendar directly via {@code new}. Left in place, their auto-scanned bean
 * definitions would still be eagerly instantiated during context refresh and fail with
 * an {@code UnsatisfiedDependencyException}, since Spring has no bean of type
 * {@code String}/{@code URI} to satisfy their constructors.
 *
 ...
 * also keeps the three affected classes free of any {@code org.springframework.*}
 * annotation -- {@link PollAndRelaySourceCalendarService} in particular lives in
 * {@code core/app}, where {@code CLAUDE.md} forbids Spring dependencies outright.
 */
final class PerCalendarComponentBeanDefinitionPruner implements BeanDefinitionRegistryPostProcessor {
    ...
    private static final Set<String> PER_CALENDAR_COMPONENT_CLASS_NAMES = Set.of(
    ...
    public void postProcessBeanDefinitionRegistry(BeanDefinitionRegistry registry) throws BeansException {
        for (var beanName : registry.getBeanDefinitionNames()) {
            ...
            if (beanClassName != null && PER_CALENDAR_COMPONENT_CLASS_NAMES.contains(beanClassName)) {
                ...
                LOG.debug(
                        "Removed auto-scanned bean definition '{}' for per-calendar component {}",
                        beanName,
                        beanClassName);
            }
        }
